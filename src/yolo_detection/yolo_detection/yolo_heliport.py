...
"""
@file       yolo_heliport.py
@brief      使用YOLOv11模型检测起降点类别和中心坐标，并发布相关话题
@details    订阅usb相机话题/image_raw，通过本地yolov11和权重文件检测起降点
            发布起降点中心坐标话题/yolo/heliport/centre
            发布起降点长宽比话题/yolo/heliport/aspect_ratio和置信度话题/yolo/heliport/confidence
@note       起降点中心坐标为相对相机主点RDF坐标系
@author     FallThrive
@date       2025-08-08
@version    1.0
"""
...
class YOLOHeliportNode(Node):
    def __init__(self):
        super().__init__('yolo_heliport')
        
        # 获取包路径
        package_share_directory = get_package_share_directory('yolo_detection')
        weights_path = os.path.join(package_share_directory, 'weights', 'yolo11m_yihe_H.pt')
        ...
        self.get_logger().info('YOLO Heliport Node has been started.')
